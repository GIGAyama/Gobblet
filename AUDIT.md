...
## 追加対応：ふりがな（2026-08-03 追記）

「アプリ上に表示されている全ての漢字にふりがなをつける」という依頼を受けて実施しました。

| # | 項目 | 判定 | 実測 |
|---|---|:--:|---|
| D14 | 画面に出る漢字にふりがなが付いている | ✅ | index.html / offline.html の本文から `<ruby>` を取り除いても漢字が残らないことを機械検査。実ブラウザ（ダイアログを開いた状態を含む）でも残存0字を確認 |
| D15 | JS の画面文言にもふりがなが付いている | ✅ | `notice()` / `showMessage()` に渡す13件すべてが `{漢字|かんじ}` 記法 |

対象にした文言（全24か所）：

- **あそびかた**の3項目（大きい駒／小さい駒／上／並べたら勝ち／自分の色／盤上の駒／動かせる／下の駒／現れて／相手／時点）
- **ボタン**「最初から」
- **フッター**「GIGA山」
- **トースト**「今は相手のばんです」「相手の駒は動かせません」「もっと大きい駒なら、そこに置けます」「この端末では大きく表示できません」
- **勝敗ダイアログ**「〇〇の勝ち！」「すごい作戦でした！」「もう一回」
- **リセット確認**「最初からやり直す？」「今の勝負はリセットされます。」「やり直す」
- **オフライン画面**「一度ひらいたことがあれば…遊べます」「下のボタンを押してみてください」

読み上げへの配慮：

- すべてのルビに `<rp>（</rp>` `<rp>）</rp>` を添え、ルビ非対応環境では「漢字（かんじ）」と読める形にしています
- トーストと「もう一回」等のボタンには、ふりがな記号を除いた素のことばを `aria-label` に入れ、
  読み上げが「おお…大きい」と濁らないようにしています
- `aria-label` などの属性（画面に出ないもの）にはふりがなを付けていません

**あわせて直した読み上げの不具合**：手駒の読み上げが「ブルーの**中さい**駒」「ブルーの**大さい**駒」という
日本語にならない文言になっていました（着手前から）。「小さい駒／中くらいの駒／大きい駒」に修正しています。
...
4. **ふりがなの読み**が、担当学年の教科書の表記と合っているか（特に「盤上（ばんじょう）」「時点（じてん）」）

## ✅ 品質ゲートの結果

`npm run check` … **合格 46 / 対象外 2 / 不合格 0**
`npm test` … **19件すべて合格**（勝敗判定・かぶせ・持ち上げて相手が勝つ場合・ふりがなの組み立て）
